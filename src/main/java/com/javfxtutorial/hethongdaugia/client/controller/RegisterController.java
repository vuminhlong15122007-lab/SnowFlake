...
public class RegisterController implements ResponseListener {
  ...
  public void clickSignUp(ActionEvent event) {
    ...
    if (password.length() < 6) {
      showWarning("Mật khẩu phải đủ tối thiểu 6 kí tự!");
      return;
    }
    // check confirm password
    if (!password.equals(confirmPassword)) {
      showWarning("Vui lòng xác thực khớp mật khẩu!");
      return;
    }
    // check so dien thoai
    if (sdt.length() != 10) {
      showWarning("Số điện thoại phải đủ 10 số!");
      return;
    }
    try {
      Long.parseLong(sdt);
    } catch (NumberFormatException e) {
      showWarning("Số điện thoại chỉ bao gồm các số!");
      return;
    }
    // check email
    if (!email.endsWith("@gmail.com")) {
      showWarning("Email phải có đuôi @gmail.com!");
      return;
    }
    ...
    new Thread(
            () -> {
              try {
                ...
                Platform.runLater(() -> showError("Không thể gửi yêu cầu đăng ký"));
              } catch (Exception e) {
    ...
    Platform.runLater(
        () -> {
          if (rp.isSuccess()) {
            ...
            delay.setOnFinished(_ -> openSignUpSuccessPopup());
            delay.play();
          } else {
            showError(rp.getMessage());
          }
        });
    NetworkManager networkManager = NetworkManager.getInstance();
