...
public class SellerManagementController implements ResponseListener {
  ...
  public void initialize() {
    ...
    if (cancelledInfoBox != null) {
      cancelledInfoBox.setVisible(false);
      cancelledInfoBox.setManaged(false);
    }
  ...
  private void showCancelledPanel(
      String icon,
      String title,
      String subtitle,
      boolean showUserCard,
      String cardColor,
      String cardTitle,
      Auction auction) {
    if (winnerInfoBox != null) {
    ...
    if (showUserCard && auction != null) {
      ...
      cancelledUserName.setText(
          nullOrEmpty(auction.getWinnerName()) ? "Null" : auction.getWinnerName());
      cancelledUserSdt.setText(
          nullOrEmpty(auction.getWinnerSdt()) ? "Null" : auction.getWinnerSdt());
      cancelledUserEmail.setText(
          nullOrEmpty(auction.getWinnerEmail()) ? "Null" : auction.getWinnerEmail());
  ...
  /**
   * Luồng xử lý khi chọn phiên: CANCELLED (có winner) → cancelledInfoBox với thông tin người vi
   * phạm CANCELLED_BY_ADMIN → cancelledInfoBox không có user card CLOSED có winner → winnerInfoBox
   * (chờ thanh toán) CLOSED không winner → cancelledInfoBox "không ai tham gia" PAID →
   * winnerInfoBox (đã thanh toán) NOT_START → detailFormBox (cho phép sửa) RUNNING → detailFormBox
   * (chỉ xem, không sửa)
   */
  ...
  private void maybeAddNotification(Auction auction) {
    ...
    if (st != AuctionStatus.CLOSED
        ...
        && st != AuctionStatus.CANCELLED_BY_ADMIN) return;
    ...
    addOrReplaceNotification(
        new SellerNotification(
            auction.getAuctionId(), type, pName, wName, auction.getWinningPrice()));
  }

  private void addOrReplaceNotification(SellerNotification notif) {
    SellerNotification existing =
  ...
  private void syncNotificationsFromAuctions(List<Auction> auctions) {
    for (Auction a : auctions) {
      ...
      if (st != AuctionStatus.CLOSED
          && st != AuctionStatus.PAID
          && st != AuctionStatus.CANCELLED
          && st != AuctionStatus.CANCELLED_BY_ADMIN) continue;
      ...
      SellerNotification notif =
          new SellerNotification(a.getAuctionId(), type, pName, wName, a.getWinningPrice());
      ...
      Optional<SellerNotification> existing =
          notifications.stream().filter(n -> n.getAuctionId() == a.getAuctionId()).findFirst();
  ...
  private void handleDeleteAuctionResponse(Response rp) {
    if ("ADMIN_DELETED_PRODUCT".equals(rp.getMessage())
