...
public class SellerManagementController implements ResponseListener {
  ...
  // ── State ─────────────────────────────────────────────────────────────────
  // Dùng shared list từ ClientModel (Doc 5) — tồn tại khi navigate qua lại
  ...
  // Base64 ảnh mặc định
  private String image = "";
  ...
  public void initialize() {
    ...
    if (filterComboBox != null) {
      filterComboBox.getItems().addAll(
          ...
          "Chờ thanh toán", "Thành toán thành công", "Sản phẩm đã hủy", "Không ai đấu giá"
      );
    ...
    productList.getSelectionModel().selectedItemProperty().addListener((obs, oldVal, newVal) -> {
      ...
      // Thông báo kết quả đấu giá (Doc 5)
        ...
        if (newVal.getWinnerId() <= 0) {
          ...
          showAlert("Kết quả đấu giá",
              "Sản phẩm \"" + newVal.getItem().getName() + "\" đã có người thắng!\n"
                  + "Người thắng: " + newVal.getWinnerName(), "Happy.gif");
        }
      }

      hienThiChiTietSanPham(newVal);
      selectedAuction = newVal;  // BUG FIX (Doc 5): set selectedAuction ngay tại đây

      saveButton.setStyle("-fx-background-color: #E67E22; -fx-text-fill: white; -fx-font-weight: bold;"
      ...
      // Sinh thông báo cho seller (Doc 5 + Doc 6)
    ...
    // Lắng nghe server push (Doc 5 + Doc 6)
  ...
  /** Xóa sạch form — tách riêng thành method (Doc 5) */
   ...
   * Doc 5: có validation thời gian + throws Exception khi lỗi.
   */
  ...
  public void hienThiChiTietSanPham(Auction auction) {
    ...
    if (cate == ItemCategory.ELECTRONICS) {
      ...
      else { brandElecField.setText(""); modelField.setText(""); }
      ...
      else { artTitleField.setText(""); artistField.setText(""); yearCreatedField.setText(""); }
      ...
      else { licensePlateField.setText(""); vehicleYearField.setText(""); brandVehicleField.setText(""); colorField.setText(""); }
    }
  }
  ...
  private void applyFilter(String filter) {
    ...
    filteredList.setPredicate(a -> {
      ...
      return switch (filter) {
        ...
        case "Sản phẩm đã hủy" -> st == AuctionStatus.CANCELLED;
  ...
  private void addOrReplaceNotification(SellerNotification notif) {
    ...
    for (SellerNotification n : notifications) {
      if (n.getAuctionId() == notif.getAuctionId()) { existing = n; break; }
    }
  ...
  /** Doc 5: dùng Popup thay vì chuyển scene (Doc 6 dùng chuyển scene — sai) */
  @FXML
  ...
  public void onResponse(Response rp) {
      ...
      Platform.runLater(() -> {
        ...
        for (Auction a : auctions) {
          ...
          if (st != AuctionStatus.CLOSED && st != AuctionStatus.PAID
              && st != AuctionStatus.CANCELLED) continue;
    ...
    if (rp.getCommand().getClass() == DeleteAuctionCommand.class) {
      ...
      Platform.runLater(() -> {
        if (rp.isSuccess()) {
          // BUG FIX (Doc 5): dùng payload từ server khi có thể
          Auction deleted = (rp.getPayLoad() instanceof Auction)
              ? (Auction) rp.getPayLoad() : selectedAuction;
    ...
    if (rp.getCommand().getClass().equals(UpdateAuctionStatusCommand.class)
      ...
      String productName = (auction.getItem() != null)
          ? auction.getItem().getName() : String.valueOf(auction.getAuctionId());
