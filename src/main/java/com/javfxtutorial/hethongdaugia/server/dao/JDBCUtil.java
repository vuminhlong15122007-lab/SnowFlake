...
public final class JDBCUtil {
  ...
  private static HikariDataSource createDataSource() {
    ...
    config.setPassword(
        configValue("snowflake.db.password", "SNOWFLAKE_DB_PASSWORD", DEFAULT_PASSWORD));
    ...
    config.setIdleTimeout(
        longConfig(
            "snowflake.db.pool.idleTimeoutMs", "SNOWFLAKE_DB_POOL_IDLE_TIMEOUT_MS", 300_000L));
    config.setMaxLifetime(
        longConfig(
            "snowflake.db.pool.maxLifetimeMs", "SNOWFLAKE_DB_POOL_MAX_LIFETIME_MS", 1_800_000L));
