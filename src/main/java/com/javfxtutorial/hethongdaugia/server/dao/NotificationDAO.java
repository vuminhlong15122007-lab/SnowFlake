...
public class NotificationDAO {
    ...
    public int insert(SellerNotification notification, int sellerId)
            throws DataInsertException {
        String sql = "INSERT INTO seller_notification"
    ...
    public void insertOrReplace(SellerNotification notification, int sellerId) throws DatabaseConnectionException, DataInsertException, DataDeleteException {

        String sql = "SELECT notification_id, type FROM seller_notification"
            ...
            try (ResultSet rs = pst.executeQuery()) { // BUG FIX: executeQuery thay executeUpdate
                if (rs.next()) {
                    ...
                    if (priority(notification.getType()) > priority(existingType)) {
                        ...
                        log.info("Đã thay thế notification id={} ({} -> {})",
                                existingId, existingType, notification.getType());
                    } else {
                        log.info("Giữ nguyên notification cũ id={} (priority {} >= {})",
                                existingId, priority(existingType), priority(notification.getType()));
                    }
                } else {
    ...
    public List<SellerNotification> findBySellerId(int sellerId) {
        String sql = "SELECT notification_id, auction_id, type, product_name, winner_name,"
        ...
        try (Connection conn = JDBCUtil.getConnection();
            ...
            try (ResultSet rs = pst.executeQuery()) { // BUG FIX: bỏ executeUpdate, dùng executeQuery
                while (rs.next()) {
                    SellerNotification n = new SellerNotification(
