        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                ItemCategory category =
                        (categoryStr != null) ? ItemCategory.valueOf(categoryStr) : null;
                Item item =
                        new Item(
                                sellerName,
                                idseller,
        ...
        switch (item.getCategory()) {
            case ART -> {
                if (!(item instanceof Art art))
                    throw new SQLException("ART nhưng object không phải Art");

                String sql =
                        "INSERT INTO art (item_id, artist, year_created, title) VALUES (?, ?, ?, ?)";
                try (PreparedStatement pst = conn.prepareStatement(sql)) {
                    ...
                    if (pst.executeUpdate() != 1)
                        throw new SQLException("Insert ART subtype failed");
                }
            }
            ...
            case ELECTRONICS -> {
                ...
                try (PreparedStatement pst = conn.prepareStatement(sql)) {
                    ...
                    if (pst.executeUpdate() != 1)
                        throw new SQLException("Insert ELECTRONICS subtype failed");
                }
            }
            ...
            case VEHICLE -> {
                ...
                try (PreparedStatement pst = conn.prepareStatement(sql)) {
                    ...
                    if (pst.executeUpdate() != 1)
                        throw new SQLException("Insert VEHICLE subtype failed");
                }
            }
    ...
    public void updateSubType(Connection conn, Item item) throws SQLException {
        switch (item.getCategory()) {
            case ItemCategory.ART -> {
                Art art = (Art) item;
                String sql =
                        "UPDATE art SET artist = ?, year_created = ?, title = ? WHERE item_id = ?";
                try (PreparedStatement pst = conn.prepareStatement(sql)) {
                    ...
                    int result = pst.executeUpdate();
                    if (result > 0)
                        log.info("Cập nhật ART thành công, itemId: {}", art.getItemId());
            ...
            case ItemCategory.ELECTRONICS -> {
                ...
                try (PreparedStatement pst = conn.prepareStatement(sql)) {
                    ...
                    int result = pst.executeUpdate();
                    if (result > 0)
                        log.info("Cập nhật ELECTRONICS thành công, itemId: {}", elec.getItemId());
            ...
            case ItemCategory.VEHICLE -> {
                ...
                try (PreparedStatement pst = conn.prepareStatement(sql)) {
                    ...
                    int result = pst.executeUpdate();
                    if (result > 0)
                        log.info("Cập nhật VEHICLE thành công, itemId: {}", vehicle.getItemId());
