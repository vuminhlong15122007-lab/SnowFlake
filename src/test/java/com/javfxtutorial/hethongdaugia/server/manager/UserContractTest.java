...
class UserContractTest {
  ...
  private User alice() {
    return new User(
        1, "alice", "pass123", "alice@example.com", "0901234567", AccountType.USER, "alice.png");
  }
  ...
  class UpdateUserProfileManagerTest {
    ...
    void updateUserProfile_sendsMergedUserToDao() throws Exception {
      ...
      try (MockedStatic<UserDAO> mockedUserDAO = mockStatic(UserDAO.class)) {
        ...
        User result =
            UserManager.getInstance().updateUserProfile(1, "new", "new@example.com", "0999", "");
    ...
    void updateUserProfile_returnsNullWhenUserDoesNotExist() throws Exception {
      ...
      try (MockedStatic<UserDAO> mockedUserDAO = mockStatic(UserDAO.class)) {
        ...
        assertNull(
            UserManager.getInstance()
                .updateUserProfile(404, "new", "new@example.com", "0999", "new.png"));
        verify(userDAO, never()).update(any(User.class));
      }
    }
  ...
  class DeleteUserManagerTest {
    ...
    void deleteUser_returnsFalseWhenUserDoesNotExistWithoutDeleting() throws Exception {
      ...
      try (MockedStatic<UserDAO> mockedUserDAO = mockStatic(UserDAO.class)) {
        ...
        assertFalse(UserManager.getInstance().deleteUser(404, "ignored", "ignored", "ignored"));
